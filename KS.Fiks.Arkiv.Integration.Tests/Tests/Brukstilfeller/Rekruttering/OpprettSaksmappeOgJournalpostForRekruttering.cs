...
namespace KS.Fiks.Arkiv.Integration.Tests.Tests.Brukstilfeller.Rekruttering
{
    ...
    public class RekrutteringTests : IntegrationTestsBase
    {
         ...
         */
        [Test, Order(1)]
        public async Task Opprett_Ledig_Stilling_Saksmappe()
        {
          ...
          /*
         * EasyCruit sitt eksempel er brukt i denne testen, mtp regel osv.
         * Denne testen oppretter journalposten med hoveddokument for en ledig stilling på saksmappe opprettet tidligere
         * Til slutt henter testen opprettet journalpost og validerer den
         */
        [Test, Order(2)]
        public async Task Opprett_Ledig_Stilling_Journalpost_Med_HovedDokument_På_Saksmappen()
        {
            // Denne id'en gjør at Arkiv-simulatoren ser hvilke meldinger som henger sammen. Har ingen funksjon ellers.  
            var testSessionId = Guid.NewGuid().ToString();
